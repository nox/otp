...
call_1(#c_call{anno=Anno}, lists, delete, [Arg1,Arg2], Sub) ->
    ...
    expr(#c_let{vars=[Item, L], arg=#c_values{es=[Arg1, Arg2]},
                body=#c_letrec{defs=[{Loop,Fun}],
                               body=#c_apply{anno=Anno, op=Loop, args=[L]}}},
         Sub);
call_1(#c_call{anno=Anno}, lists, dropwhile, [Arg1,Arg2], Sub) ->
    Loop = #c_var{name={'lists^dropwhile',1}},
    F = #c_var{name='F'},
    Rest = #c_var{name='Rest'},
    Xs = #c_var{name='Xs'},
    X = #c_var{name='X'},
    B = #c_var{name='B'},
    Err1 = #c_tuple{es=[#c_literal{val='case_clause'}, X]},
    ...
    CC3 = #c_clause{pats=[X], guard=#c_literal{val=true},
                    body=match_fail(Anno, Err1)},
    Case = #c_case{arg=B, clauses=[CC1, CC2, CC3]},
    C1 = #c_clause{pats=[#c_alias{var=Rest, pat=#c_cons{hd=X, tl=Xs}}],
                   guard=#c_literal{val=true},
                   body=#c_let{vars=[B],
                               arg=#c_apply{anno=Anno, op=F, args=[X]},
                               body=Case}},
    ...
    Err2 = #c_tuple{es=[#c_literal{val='function_clause'}, F, Xs]},
    C3 = #c_clause{pats=[Xs], guard=#c_literal{val=true},
                   body=match_fail([{function_name,{'lists^dropwhile',1}}|Anno], Err2)},
    Fun = #c_fun{vars=[Xs],
                 body=#c_case{arg=Xs, clauses=[C1, C2, C3]}},
    L = #c_var{name='L'},
    expr(#c_let{vars=[F, L], arg=#c_values{es=[Arg1, Arg2]},
...
call_1(#c_call{anno=Anno}, lists, nth, [Arg1,Arg2], Sub) ->
    ...
    expr(#c_let{vars=[N, L], arg=#c_values{es=[Arg1, Arg2]},
         ...
         Sub);
call_1(#c_call{anno=Anno}, lists, nthtail, [Arg1,Arg2], Sub) ->
    Loop = #c_var{name={'lists^nthtail',2}},
    N = #c_var{name='N'},
    Xs = #c_var{name='Xs'},
    X = #c_var{name='X'},
    One = #c_literal{val=1},
    ...
    C2 = #c_clause{pats=[#c_tuple{es=[N, #c_cons{hd=X, tl=Xs}]}],
                   ...
                   body=#c_let{vars=[N],
                               ...
                               body=#c_apply{anno=Anno,
                                             op=Loop,
                                             args=[N, Xs]}}},
    C3 = #c_clause{pats=[#c_tuple{es=[#c_literal{val=0}, Xs]}],
                   guard=#c_call{module=#c_literal{val=erlang},
                                 name=#c_literal{val=is_list},
                                 args=[Xs]},
                   body=Xs},
    Err = #c_tuple{es=[#c_literal{val=function_clause}, N, Xs]},
    C4 = #c_clause{pats=[#c_tuple{es=[N, Xs]}], guard=#c_literal{val=true},
                   body=match_fail([{function_name,{'lists^nthtail',2}}|Anno], Err)},
    Fun = #c_fun{vars=[N, Xs],
                 body=#c_case{arg=#c_tuple{es=[N, Xs]},
                 clauses=[C1, C2, C3, C4]}},
    L = #c_var{name='L'},
    expr(#c_let{vars=[N, L], arg=#c_values{es=[Arg1, Arg2]},
                body=#c_letrec{defs=[{Loop,Fun}],
                               body=#c_apply{anno=Anno, op=Loop, args=[N, L]}}},
         Sub);
call_1(#c_call{anno=Anno}, lists, prefix, [Arg1,Arg2], Sub) ->
    Loop = #c_var{name={'lists^prefix',2}},
    Xs = #c_var{name='Xs'},
    X = #c_var{name='X'},
    Ys = #c_var{name='Ys'},
    Y = #c_var{name='Y'},
    YsIsList = #c_call{module=#c_literal{val=erlang},
                       name=#c_literal{val=is_list},
                       args=[Ys]},
    C1 = #c_clause{pats=[#c_tuple{es=[#c_cons{hd=X, tl=Xs},
                                      #c_cons{hd=Y, tl=Ys}]}],
                   guard=#c_call{module=#c_literal{val=erlang},
                                 name=#c_literal{val='=:='},
                                 args=[X, Y]},
                   body=#c_apply{anno=Anno, op=Loop, args=[Xs, Ys]}},
    C2 = #c_clause{pats=[#c_tuple{es=[#c_literal{val=[]}, Ys]}],
                   guard=YsIsList,
                   body=#c_literal{val=true}},
    C3 = #c_clause{pats=[#c_tuple{es=[#c_cons{hd=X, tl=Xs}, Ys]}],
                   guard=YsIsList,
                   body=#c_literal{val=false}},
    Err = #c_tuple{es=[#c_literal{val=function_clause}, Xs, Ys]},
    C4 = #c_clause{pats=[#c_tuple{es=[Xs, Ys]}], guard=#c_literal{val=true},
                   body=match_fail([{function_name,{'lists^prefix',2}}|Anno], Err)},
    Fun = #c_fun{vars=[Xs, Ys],
                 body=#c_case{arg=#c_tuple{es=[Xs, Ys]},
                 clauses=[C1, C2, C3, C4]}},
    expr(#c_let{vars=[Xs, Ys], arg=#c_values{es=[Arg1, Arg2]},
                body=#c_letrec{defs=[{Loop,Fun}],
                               body=#c_apply{anno=Anno,
                                             op=Loop,
                                             args=[Xs, Ys]}}},
