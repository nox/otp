...
uexpr(#itry{anno=A,args=As0,vars=Vs,body=Bs0,evars=Evs,handler=Hs0}, Ks, St0) ->
    ...
    NewAs1 = new_in_any(As1),
    {Bs1,St2} = uexprs(Bs0, union(NewAs1, Ks), St1),
    {Hs1,St3} = uexprs(Hs0, Ks, St2),
    Used = intersection(used_in_any(Bs1++Hs1++As1), Ks),
    New = intersection(union(NewAs1, new_in_all(Bs1)), new_in_all(Hs1)),
...
cexpr(#ireceive2{anno=A,clauses=Lcs,timeout=Lto,action=Les}, As, St0) ->
    ...
    {#c_receive{anno=A#a.anno,
     ...
     Exp,A#a.us,St3};
cexpr(#itry{anno=A,args=La,vars=Vs0,body=Lb,evars=Evs,handler=Lh}, As, St0) ->
    Exp = intersection(A#a.ns, As),           %Exports
    {Cb,Usb,St1} = cexprs(Lb, Exp, St0),
    Expa = intersection(union(Exp, Usb), new_in_any(La)),
    Vs1 = Vs0++make_vars(Expa),
    {Ca,_Us1,St2} = cexprs(La, Expa, St1),
    {Ch,_Us3,St3} = cexprs(Lh, Exp, St2),
    {#c_try{anno=A#a.anno,arg=Ca,vars=Vs1,body=Cb,evars=Evs,handler=Ch},
     Exp,A#a.us,St3};
